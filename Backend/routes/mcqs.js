...
router.post("/add-mcq", async (req, res) => {
    ...
    // Validation check
    if (!subject || !chapter || !question || options.length !== 4 || !explanation) {
    ...
    try {
        ...
        if (existingMcq) {
            return res.status(400).json({ message: "MCQ with this question already exists in the chapter." });
        }
...
router.get("/:subject/:chapter", async (req, res) => {
    ...
    try {
        ...
        if (mcqs.length === 0) {
            return res.status(404).json({ message: "No MCQs found for this subject and chapter." });
        }
        res.json(mcqs);
    } catch (error) {
