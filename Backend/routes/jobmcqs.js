...
router.post("/add-job-mcq", async (req, res) => {
  ...
  try {
    ...
    const existingJobMcq = await JobMcq.findOne({
      jobTitle,
      chapter,
      question,
    });
    if (existingJobMcq) {
      return res.status(400).json({
        message: "Job MCQ with this question already exists in the chapter.",
      });
    }
    ...
    return res
      .status(201)
      .json({ message: "Job MCQ added successfully", mcq: newJobMcq });
  } catch (error) {
...
router.get("/:jobTitle/total-mcqs", async (req, res) => {
  const { jobTitle } = req.params;

  try {
    const totalJobMcqs = await JobMcq.countDocuments({ jobTitle });
    res.json({ jobTitle, totalJobMcqs });
  } catch (error) {
    console.error("Error counting Job MCQs:", error);
    res.status(500).json({ message: "Error counting Job MCQs", error });
  }
});
...
router.get("/:jobTitle/:chapter", async (req, res) => {
  ...
  try {
    ...
    if (jobMcqs.length === 0) {
      return res
        .status(404)
        .json({ message: "No Job MCQs found for this job title and chapter." });
    }
    res.json(jobMcqs);
  } catch (error) {
